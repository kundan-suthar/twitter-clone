    ...
    return (
        <article className="px-4 py-3 border-b border-zinc-800 hover:bg-white/[0.03] transition-colors cursor-pointer flex gap-3">
            ...
            <div className="shrink-0">
                <img src={avatar} alt={name} className="w-10 h-10 rounded-full bg-zinc-800 object-cover" />
            </div>
            ...
            <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 truncate text-[15px]">
                        <span className="font-bold text-[#e7e9ea] truncate">{name}</span>
                        <span className="text-zinc-500 truncate">{handle}</span>
                        <span className="text-zinc-500">·</span>
                        <span className="text-zinc-500 hover:underline">{timestamp}</span>
                    </div>
